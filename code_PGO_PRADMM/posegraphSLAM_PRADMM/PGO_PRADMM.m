%% ADMM for pose graph optimization
function result = PGO_PRADMM(vertex,edge,Ad_Mat,opts)

%% pose graph optimization

% Relaxed dual stepsize
check_tau = 1.4;
...
while(stopc > tol && k <= MaxIter)
    ...
    %% update p^k+1
        if check_p_subproblem == 1  
            for i = 2 : num_v
                ...
                for l=1:num_col
                    M2=matrixW(edge(col2(l),6:9))*matrixW(q(:,col1(l))).*[1,-1,-1,-1];
                    s2=[1,0,0,0]';
    ...
    for i = 2 : num_v
        times.tttu = tic;
        [row1,row2]=Adjacency_Mat_row(Ad_Mat,i);   % out
        p_M=zeros(3,3);
        p_s=zeros(3,1);
        num_row=length(row1);
        for j=1:num_row
